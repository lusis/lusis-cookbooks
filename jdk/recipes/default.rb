...
current_java_version_pattern = (node.jdk.install_flavor == 'sun') ? /Java HotSpot\(TM\)/ : /^OpenJDK/
...
execute "update-java-alternatives" do
  command "update-java-alternatives --jre -s java-6-#{node["jdk"]["install_flavor"]}"
  returns 0
...
node.run_state[:java_pkgs].each do |pkg|
  package pkg do
    ...
    if platform?("ubuntu", "debian")
      if node.jdk.install_flavor == "sun"
